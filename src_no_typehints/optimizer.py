...
class Optimizer:
    ...
    @property
    def shown_cc_offdiag_sumsqrs(self)  :
    ...
    def update_opts(self, use_functions_dict, max_functions):
    ...
    def trim_composite_function_list(self,status_bar:tk_label):
        ...
        # Only run this at the end of the tree generation,
    ...
    def fit_setup(self, fourier_condition = True)  :
    ...
    # does not change top5 lists
    def fit_this_and_get_model_and_covariance(self, model_ ,
...
def correlation_from_covariance(cov_mat)  :
    ...
    return correlation
def coefficient_of_covariance(cov_mat, mean_list)  :
    cc = cov_mat.copy()
...
def sum_sqr_off_diag(cc)  :
    sumsqr = 0
    for (idx, jdx), cc_ij in np.ndenumerate(cc):
        if idx < jdx :
            sumsqr += cc_ij**2
    return sumsqr
def list_sums_weights(l1,l2,w1,w2)  :
    if len(l1) != len(l2) :
        return []
    sum_list = []
