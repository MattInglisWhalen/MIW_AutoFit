"""
Reads data from file and keeps track of its manipulatios/properties
"""
...
class DataHandler:
    ...
    def __init__(self, filepath):
        ...
        self._X0: float = 1.0
        self._logy_flag: bool = False
        self._Y0: float = 1.0

        # TODO:
        # consider adding an optimizer instance to the DataHandler class, so that each file's
        # optimization can only be accessed through the file handler
            ...
            for datum in self._data:
                if self._histogram_flag:
                    sigma_lower, sigma_upper = (
                        datum.assym_sigma_pos[0] / datum.pos,
                        datum.assym_sigma_pos[1] / datum.pos,
                    )
        ...
        if not new_flag and self._logx_flag:
            ...
            if self._histogram_flag:
                ...
                for datum in self._data:
                    ...
                    if self._histogram_flag:
                        sigma_lower, sigma_upper = (
                            datum.pos * datum.assym_sigma_pos[0],
                            datum.pos * datum.assym_sigma_pos[1],
                        )
                        datum.assym_sigma_pos = (sigma_lower, sigma_upper)
                    else:
                        datum.sigma_pos = datum.sigma_pos * datum.pos
    ...
    def unlogged_sigmax_data(self) -> list[float]:
        ...
        return np.array(
            [
                (
                    datum.assym_sigma_pos[0] * self._X0 * np.exp(datum.pos),
    ...
    def calc_entries_per_line(self, delim: str) -> int:
        with open(self._filepath, mode="r", encoding="utf-8") as file:
            for line in file:
                ...
                if regex.search("[a-zA-Z]", data_str[0]):
                    self._header_flag = 1
    ...
    @staticmethod
    def cleaned_line_as_str_list(line, delim: str) -> list[str]:
        data = regex.split(r"\s*" + delim + r"\s*", line[:-1])
        while data[-1] == "":
            data = data[:-1]
        return data

    def read_csv(self, delim: str = ","):
    ...
    def read_as_scatter(self, delim: str):
        ...
        with open(self._filepath, mode="r", encoding="utf-8") as file:
            ...
            # it's messy to repeat the logic and loop, but it's
            # also inefficient to have an `if` in a `for loop`
    ...
    def read_as_histogram(self, delim: str):
        # should probably be using pandas pd.read_csv()
        vals = []
        with open(self._filepath, mode="r", encoding="utf-8") as file:
    ...
    def make_histogram_data_from_vals(self, vals):
        ...
        if minval - np.floor(minval) < 2 / count:
            ...
            # then use the integers as a bin boundary
            minval = minval // 1
        ...
        if self._logx_flag:
            hist_counts, hist_bounds = np.histogram(
                vals, bins=np.geomspace(minval, maxval, num=num_bins + 1)
            )
            if min(hist_counts) == 0 and self._logy_flag:
                logger(
                    "In make_histogram_data, your can't x-log for this data because you're already"
                    " y-logging, and you can't take the log of 0."
                )
                self._logx_flag = False
        # this used to be         if not self.logx_flag :
        else:
            hist_counts, hist_bounds = np.histogram(
                vals, bins=np.linspace(minval, maxval, num=num_bins + 1)
            )
            ...
            for idx, count in enumerate(hist_counts):
                ...
                self._data.append(
                    Datum1D(
                        ...
                        val=count,
                        assym_sigma_pos=(
                            geom_mean - hist_bounds[idx],
                            hist_bounds[idx + 1] - geom_mean,
                        ),
    ...
    def excel_range_as_list_of_idx_tuples(excel_vec):
        ...
        if excel_vec == "":
            ...
            return []
        try:
            left, right = regex.split(":", excel_vec)
        except ValueError as exc:
            logger(f"{excel_vec=}")
            raise ValueError from exc
        # logger(f"{left=} {right=}")
        left_chars = regex.split("[0-9]", left)[0]
        left_ints = regex.split("[A-Z]", left)[-1]

        right_chars = regex.split("[0-9]", right)[0]
        right_ints = regex.split("[A-Z]", right)[-1]

        if left_chars == right_chars:
            # A1 denotes ColRow so transpose the two
            return [
                (idx, DataHandler.excel_chars_as_idx(left_chars))
                for idx in range(
        ...
        if left_ints == right_ints:
            return [
                (DataHandler.excel_ints_as_idx(left_ints), idx)
                for idx in range(
                    DataHandler.excel_chars_as_idx(left_chars),
    ...
    def excel_cell_as_idx_tuple(excel_cell_str):
        ...
        chars = regex.split("[0-9]*", excel_cell_str)[0]
        ints = regex.split("[A-Z]*", excel_cell_str)[-1]
    ...
    def read_excel_as_scatter(self):
        ...
        xvals = []
        its = DataHandler.excel_range_as_list_of_idx_tuples(self._x_column_endpoints)
        for idx, loc in enumerate(its):
            try:
                ...
                logger(data_frame)
                logger(
                    f"Provided spreadsheet range is invalid for spreadsheet at {self._filepath}."
                )
        ...
        yvals = []
        its = DataHandler.excel_range_as_list_of_idx_tuples(self._y_column_endpoints)
        for idx, loc in enumerate(its):
            try:
                val = data_frame.iloc[loc[0], loc[1]]
            except IndexError:
        ...
        if DataHandler.valid_excel_endpoints(self._sigmax_column_endpoints):
            ...
            for idx, loc in enumerate(its):
                try:
                    ...
                    logger(data_frame)
                    logger(
                        f"Provided spreadsheet range is invalid for spreadsheet "
                        f"at {self._filepath}."
                    )
                if idx == 0 and regex.search("[a-zA-Z]", str(val)):
                    pass
    ...
    def read_excel_as_histogram(self):
        ...
        for idx, loc in enumerate(
            DataHandler.excel_range_as_list_of_idx_tuples(self._x_column_endpoints)
        ):
            try:
                val = data_frame.iloc[loc[0], loc[1]]
            except IndexError:
    ...
    def normalize_histogram_data(self, error_handler=print) -> bool:
        ...
        for datum in self._data:
            ...
            if self._logx_flag:
                # relies on histogram asymmetric x-errors corresponding
                # to the upper and lower bin widths
