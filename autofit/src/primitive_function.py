"""Implements the PrimitiveFunction class"""
...
class PrimitiveFunction:
    ...
    _callable_1_param : a callable function of the form f(x), e.g. np.exp(x).
                        This is then converted into a _func
    ...
    def __init__(
        self,
        func: Union[None, Callable[[float, float], float]] = None,
        name: str = "",
        arg: float = 1.0,
        other_callable: Union[None, Callable[[float], float]] = None,
    ):
        """Initializes the PrimitiveFunction"""
        ...
        self._func: Callable[[float, float], float] = (
            func  # valid functions must all be of the type f(x,arg)
        )
    ...
    def __repr__(self):
        if self._callable_1param is not None:
            return (
                f"Prim {self._name} uses arg*{self._callable_1param.__name__}"
                f"(x) with coefficient {self._arg}"
            )
    ...
    def n_exp_dim2(x, arg) -> float:  # for Normal Composite
        if arg > 0:
            return np.exp(-(x**2) / (2 * arg**2)) / np.sqrt(2 * np.pi * arg**2)
        return 1e5 * x + 1e50

    # arbitrary powers can be created using function composition
    # (i.e using the CompositeFunction class)
    ...
    def built_in_list() -> list[PrimitiveFunction]:
        ...
        for prim in PrimitiveFunction._built_in_prims_dict.values():
