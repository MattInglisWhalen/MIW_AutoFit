"""
Implementing the CompositeFunction class.
"""
...
# ^ if github actions failing, .primitive_functions works
from autofit.src.package import logger


class CompositeFunction:
    """
    This class represents the nodes of the tree. A node with no children is just a wrapper
    for a primitive function, while a node with no parent or older brothers is the function
    tree as a whole.
    """
    ...
    A composite function is represented as a tree.
      ...
      /\    \     \
     2  3   5      7
    Branches (children) sharing the same parent node are summed together and leaves (brothers) 
    sharing the same branch are multiplied together. All brothers on a branch know their parent, 
    but the parent only knows the oldest brother on each branch. Branches and leaves on the tree 
    implement and model functional composition stemming from the parent node. E.g., with reference 
    to the above tree
    ...
    def __init__(
        ...
        # A list of (idx1, func, idx3) triplets, with the interpretation
        # that par[idx1] = func( par[idx2 )]. If there are constraints, the add_child and
        # add_brother functions should throw an error
    ...
    def print_sub_facts(self):
        """Prints submodel info about this CompositeFunction"""
        logger(
            f"{self}{'is ' if self._is_submodel else 'isnt '} "
            f"submodel of {self._submodel_of} "
            f"{'' if self._submodel_zero_index < 0 else self._submodel_zero_index}"
        )
    ...
    def add_child(
        self,
        child: Union[PrimitiveFunction, CompositeFunction],
        update_name: bool = True,
    ):
        """Add a child function to (function composition inside) this CompositeFunction"""
        ...
        if isinstance(new_child, PrimitiveFunction):  # have to convert it to a Composite
            ...
            new_child.parent = self  # .parent setter has side effects, is this desired?
            self._children_list.append(new_child)

        if update_name:
            self.build_longname()

    def add_younger_brother(
        self,
        brother_to_add: Union[PrimitiveFunction, CompositeFunction],
        update_name=True,
    ):
        """Add a younger brother to (multiplication with) this CompositeFunction."""
    ...
    def copy(self) -> CompositeFunction:
        """Deep copy of this CompositeFunction"""
    ...
    def build_longname(self):
        """Updates the longname of this CompositeFunction"""
        ...
        if (
            ...
            and self.prim.name == "sum_"
            and self.younger_brother is None
        ):
    ...
    def get_nodes_with_freedom(self, skip_flag=0):
        ...
        for idx_constrained, _, _ in sorted(
            self._constraints, key=lambda tup: tup[0], reverse=True
        ):
    ...
    def tree_as_string(self, buffer_chars=""):
        ...
        bools = (
            ...
            self.older_brother is not None,
            self.younger_brother is not None,
        )
    ...
    def tree_as_string_with_args(self, buffer_chars=""):
        ...
        # pads and truncates to ensure a length of 10
        info_str = f"{self._prim.arg:+.2E}{self._prim.name[:10] : <10}"
    ...
    def tree_as_string_with_dimensions(self, buffer_chars=""):
        ...
        for idx, child in enumerate(self._children_list):
            if idx > 0:
                tree_str += buffer_chars + next_buffer
            tree_str += child.tree_as_string_with_dimensions(
                buffer_chars=buffer_chars + next_buffer
            )

        if self.younger_brother is not None:
            tree_str += "\n" + buffer_chars
            bro_str = self._younger_brother.tree_as_string_with_dimensions(
    ...
    def depth(self) -> int:
        ...
        for child in self._children_list:
            ...
            curr_max = max(curr_max, child_depth)
        return 1 + curr_max
    ...
    def has_reciprocal_cancel_pospow(self):
        if (
            self.prim.name == "pow_neg1"
            and self.younger_brother is not None
    ...
    def has_log_with_odd_power(self):
        if (
            ...
            if child.younger_brother is None or (
                child.younger_brother.prim.name
                in ["pow_neg1", "pow3"]  # log(pow_neg1 . pow_neg1) = log(pow1^2)
    ...
    def has_pow_with_no_sum(self):  # exludes cos, sin, log
        if (
            self.prim.name[:3] == "pow"
            and self.num_children() == 1
            and self.children_list[0].prim.name[:4]
            in ["my_e", "pow0", "pow1", "pow_", "pow2", "pow3", "pow4"]
    ...
    def has_exp_with_pow0(self):
        if self.prim.name == "my_exp" and "pow0" in [
            icomp.prim.name for icomp in self._children_list
        ]:
            return True
    ...
    def eval_at(self, x, X0=0, Y0=0):
        ...
        if Y0:
            # the model is working with LY as the dependent variable,
            # but we're expecting to return x
    ...
    @staticmethod
    def construct_model_from_str(
        form: str, error_handler: Callable[[str], bool], name: str = ""
    ) -> Union[None, CompositeFunction]:

        # logger(f"Entering construction for {form}")

        # get all the names of the primitives used
        split_form = regex.split("[·+()*]", form)
        ...
        error_handler(" ")
        scipy_continuous = (
            scipy.stats._continuous_distns  # pylint: disable=protected-access
        )  # pylint: disable=protected-access
        for prim in prim_names:
            valid = False
            library_list = [np, scipy.special, scipy_continuous]
            library_names = [
                "numpy",
                "scipy.special",
                "scipy.stats",
            ]  # math doesn't accept arrays of xvals as inputs
            ...
            if not valid and prim in PrimitiveFunction.built_in_dict():
                ...
                try:
                    ...
                    error_handler(
                        f"<autofit> function {prim} has multiple arguments, "
            ...
            for lib, lib_name in zip(library_list, library_names):
                ...
                if getattr(lib, prim, None) is not None:
                    ...
                    try:
                        ...
                        error_handler(
                            f"<{lib_name}> function {prim} has multiple arguments, "
                            f"or only takes integers as input."
                        )
                    else:
                        if lib == scipy_continuous:
            ...
            if not valid:
                logger(
                    "CompositeFunction.construct_model_from_str(): dict=",
                    PrimitiveFunction.built_in_dict().keys(),
                )
                logger(
                    f"CompositeFunction.construct_model_from_str(): >{split_form}<",
                    f"{prim_names}",
                )
                error_handler(
                    f"CompositeFunction.construct_model_from_str(): Couldn't find a valid version "
                    f"of {prim} in the list of known functions."
                )
                return None
        ...
        while form_it < len(form):
            ...
            if c == "(":
                ...
                sub_model = CompositeFunction.construct_model_from_str(
                ...
                if form_it + 1 == len(form) or form[form_it + 1] in ["+", "·", "*"]:
                    for prim in prim_list[:]:
                        ...
                        error_handler(
                            f"Couldn't find primitive for {prim_to_add_name} in {prim_list}"
                        )
                        return None
                    if last_char == "+":
                    ...
                    while open_paren2 > 0:
                        ...
                        if form_it + 1 == len(form):
                            break
                    prim_with_composition = CompositeFunction.construct_model_from_str(
                        prim_to_add_name, error_handler=error_handler
                    )
                    composed_fn = CompositeFunction.construct_model_from_str(
                        composed_fn_name, error_handler=error_handler
                    )
                    if prim_with_composition is None or composed_fn is None:
                        error_handler(
                            f"{form} had a problem creating sub-function "
        ...
        if (
            man_model.prim.name == "sum_"
            and man_model.num_children() == 1
            and man_model.younger_brother is None
        ):
    ...
    def replace_node(
        ...
        # untested with siblings
        parent_of_removed = old_node.parent
        parent_of_removed.children_list.remove(old_node)
        parent_of_removed.add_child(new_node)
    ...
    def build_built_in_dict() -> None:
        ...
        gaussian = CompositeFunction(
            prim_=PrimitiveFunction.built_in("exp"),  # A
        ...
        sigmoid = CompositeFunction(
            prim_=PrimitiveFunction.built_in("sum"),  # F
    ...
    def built_in_list() -> list[CompositeFunction]:
        ...
        for comp in CompositeFunction.built_in_dict().values():
    ...
    def built_in(key) -> CompositeFunction:
        ...
        if key[:10] == "Polynomial":
            ...
            if degree == 0:
                return CompositeFunction(
                    prim_=PrimitiveFunction.built_in("pow0"), name="Polynomial0"
                )
    ...
    def dimension_func(self) -> int:
        ...
        if not self._children_list:
            if self._prim.name[:4] == "pow1":
                return 1
            if self._prim.name[:4] == "pow2":
                return 2
            if self._prim.name == "pow3":
                return 3
            if self._prim.name == "pow4":
                return 4
            if self._prim.name[:8] == "pow_neg1":
                return -1
            if self._prim.name[:3] == "Pow":
                return int(self._prim.name[3:])
            if self._prim.name == "sum_":
