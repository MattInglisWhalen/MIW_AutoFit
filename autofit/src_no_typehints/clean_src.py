"""
My version of  MacOSX is super old and can only support Python 3.6.
It therefore doesn't like type hints so this script lets me clear the typehints away,
allowing for compilation on iOS
"""
        ...
        with open(filename, "r") as infile, open(cleaned_files + "/" + filename, "w") as outfile:
            for line in infile:
                ...
                stripped_line_2 = regex.sub(
                    "Callable\[[,a-zA-Z\[\]\s]*]", "Callable", stripped_line_1
                )
                ...
                # no type hinting for function args -- clash with array slicing
                stripped_line_8 = regex.sub(":[.,a-zA-Z\s]*\)", ")", stripped_line_7)

                # no type hinting for function args -- clash with array slicing
                stripped_line_9 = regex.sub(":[.,a-zA-Z\s]*=", "=", stripped_line_8)

                # python 3.6 has no {var=} in fstrings
                stripped_line_99 = regex.sub("=\}", "}", stripped_line_9)

                # python 3.6 has no {var=} in fstrings
                stripped_line = regex.sub(
                    "from __future__ import annotations", "", stripped_line_99
                )
