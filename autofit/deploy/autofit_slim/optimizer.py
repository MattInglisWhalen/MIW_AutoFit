...
from .package import logger, DEV
...
class Optimizer:
    ...
    def __init__(
        ...
        self._data: list[Datum1D] = (
            []
        )  # the raw datapoints of (x,y), possibly with uncertainties. It may
    ...
    def update_top5_rchisqrs_for_new_data(self, new_data):
        logger(
            f"Optimizer.update_top5_rchisqrs_for_new_data(): Updating top5 criterions. Before: {self.top5_rchisqrs}"
        )
        self.set_data_to(new_data)
        for idx, model in enumerate(self.top5_models):
        ...
        for idx, (topper, chisqr) in enumerate(
            ...
            if abs(rchisqr - chisqr) < 1e-5:
                ...
                if topper.depth > model.depth:
                    logger(
                        f"Booting out contender {model.name} with depth {model.width} "
    ...
    def build_composite_function_list(self, status_bar):
        ...
        if not self._regen_composite_flag:
            return
        logger(
            f"{self._regen_composite_flag}, so regenerating composite list with {self._max_functions} "
    ...
    def validate_fails(self, icomp):
        ...
        good_list = [
            ...
            # "my_exp(pow2(my_log))",
            "my_sin(pow0+my_exp(pow1))," "pow4+pow1",
        ]
        ...
        if (
            name == "pow1·my_exp(my_exp)" or name == "pow1·my_exp(pow1·my_exp)"
        ) and not remove_flag:
    ...
    def fit_loop(
        ...
        try:
            better_guess, better_cov = curve_fit(
        ...
        if any(
            ...
            for it in range(3):  # iterate 3 times
                ...
                model.args = list(np_pars)
                logger(
                    f"Now after {it+1} refits, with effective variance: {np_pars} +- {np.sqrt(np.diagonal(np_cov))}"
                )
    ...
    def fit_this_and_get_model_and_covariance(
        ...
        if init_guess is None:
            while model.is_submodel:
                ...
                logger(
                    f"rchisqr for supermodel= {self.reduced_chi_squared_of_fit(supermodel)} {do_halving=} {halved=}"
                )
    ...
    def set_estimated_std(self, model: CompositeFunction):
        ...
        logger(
            f"{np.finfo(np.float64).max} Optimizer.set_estimated_std() Estimating std as {est_std} from "
        ...
        for i, argi in enumerate(opt_args):
            for j, argj in enumerate(opt_args):
                ...
                invCovIJ = (
                    ...
                    * dir_bigger[j]
                    * 0.5
                    * (1 / mid_delta**2 - invV[i, i] - invV[j, j])
                )
                invV[i, j] = invCovIJ
                invV[j, i] = invCovIJ
    ...
    def create_cos_sin_frequency_lists(self):
        ...
        x_points = np.linspace(
            minx, maxx, num=len(self._data) + 1
        )  # supersample of the data, with endpoints
        ...
        for n in range(min(len(pos_nu), 10)):
            ...
            min_idx, min_val = np.argmin(sin_rchisqr_list[1:-1]), min(
                sin_rchisqr_list[1:-1]
            )  # don't look at endpoints
            if min_val < best_rchisqr:
                list_idx = min_idx + 1
                if (
                    min_val < sin_rchisqr_list[list_idx - 1]
                    and min_val < sin_rchisqr_list[list_idx + 1]
                ):
                    # we found a minimum
            ...
            # now with cosine
            cos_func = CompositeFunction(
                prim_=PrimitiveFunction.built_in("cos"),
                children_list=[PrimitiveFunction.built_in("pow1")],
            )
            test_func2 = CompositeFunction(
                prim_=PrimitiveFunction.built_in("sum"),
            ...
            min_idx, min_val = np.argmin(cos_rchisqr_list[1:-1]), min(
                cos_rchisqr_list[1:-1]
            )  # don't look at endpoints
            if min_val < best_rchisqr:
                is_sin = False
                list_idx = min_idx + 1
                if (
                    min_val < cos_rchisqr_list[list_idx - 1]
                    and min_val < cos_rchisqr_list[list_idx + 1]
                ):
                    # we found a minimum
                    best_freq = pos_nu[argmax] + delta_nu * (list_idx - 3) / 3
                else:
    ...
    def find_initial_guess_scaling(self, model):
        ...
        weighted_point_norm = list_sums_weights(
            weighted_point, [0 for _ in scaling_args_no_sign], 1 / weighted_norm, 1
        )
        model.set_args(*weighted_point_norm)
        temp_rchisqr = self.reduced_chi_squared_of_fit(model)
        if (
            temp_rchisqr < best_rchisqr and model.num_trig() < 1
    ...
    def find_set_initial_guess_scaling(self, composite: CompositeFunction):
        ...
        if composite.parent is None:
            ...
            slope_at_zero = (composite.eval_at(2e-5) - composite.eval_at(1e-5)) / (
                1e-5 * composite.prim.arg
            )
    ...
    def make_fit_image(self, model=None):
        ...
        self.fix_axes_labels(
            axes, min(x_points), max(x_points), min(lower_bar), max(upper_bar), "x"
        )
